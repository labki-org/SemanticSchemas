...
class FormGenerator {
	...
	public function generateForm( EffectiveCategoryModel $category ): string {
		...
		$lines[] = 'This is the form for editing [[:Category:' . $name . ']] pages.';

		$formInput = [
			'form=' . $name,
			'autocomplete on category=' . $name,
		];

		if ( $category->getTargetNamespace() !== null ) {
			$formInput[] = 'namespace=' . $category->getTargetNamespace();
		}
		...
		if ( $category->getTargetNamespace() !== null ) {
			...
			$lines[] = '{{{info|page name=' . $category->getTargetNamespace() . ':<page name>}}}';
			$lines[] = '';
		}
	...
	private function generateTableField( FieldModel $field ): array {
		...
		$out[] = '| {{{field|' . $param . '|property=' . $prop->getName() . '|' . $input . '}}}';
